...
\end{figure}
%
Trong Hình~\ref{fig:restyle-encoder-1}, tôi sử dụng ảnh thật (1) lấy từ tập dữ liệu FFHQ và tạo ảnh giả tương ứng (2) bằng phương pháp ReStyle~\cite{alaluf2021restyle}. 
%
Việc sử dụng cặp ảnh thật–giả có cùng đặc điểm khuôn mặt giúp tôi khảo sát trực quan độ khó của nhiệm vụ phân biệt hình ảnh tạo sinh. Kết quả cho thấy hai ảnh có mức độ tương đồng cao về hình dáng và chi tiết, gây khó khăn cho việc phân biệt bằng mắt thường.
%
...
Các nghiên cứu trước đây \textcolor{red}{trích dẫn} cũng chỉ ra rằng các đặc trưng nhân tạo có xu hướng tập trung ở vùng tần số cao. Một số phương pháp đã tận dụng điều này để phát triển bộ phân loại dựa trên đặc trưng tần số \textcolor{red}{trích dẫn}. Ta tiến hành một thử nghiệm nhỏ nhằm đánh giá sự ảnh hưởng của đặc trưng tần số đến hiệu xuất bộ phân loại ảnh thật, giả.



\subsection{Thử nghiệm 2}

Bước tiếp theo của quá trình phân tích, ta thực hiện huấn luyện hai bộ phân loại dựa trên kiến trúc ResNet và sử dụng cùng bộ dữ liệu \textcolor{red}{[Tên tập dữ liệu]}.

\textbf{Bộ phân loại số 1:} (Original RGB ) ảnh đầu vào là ảnh màu RGB gốc, giữ nguyên toàn bộ thông tin tần số.  

\textbf{Bộ phân loại số 2:} (Highpass 50\%) Được huấn luyện trên cùng kiến trúc với Mô hình 1, nhưng ảnh đầu vào đã được xử lý bằng bộ lọc thông cao, loại bỏ 50\% thành phần tần số thấp bằng cách sử dụng biến đổi Fourier. Kết quả huấn luyện được trình bày trong hình~\ref{fig:Experiment_Highpass50_percent}
...
%
Làm như vậy không chỉ giúp giảm độ phức tạp tính toán mà còn phù hợp hơn cho các hệ thống yêu cầu xử lý nhanh, có tài nguyên hạn chế hoặc dễ dàng tính toán song song nếu hệ thống hổ trợ GPU.
...
Trong đề tài này, mục tiêu hướng đến là xây dựng một phương pháp có cấu trúc đơn giản, tối ưu về tốc độ xử lý, và phù hợp với các hệ thống có tài nguyên hạn chế trong nhiệm vụ phát hiện hình ảnh tạo sinh. Để đạt được mục tiêu này, kiến trúc ResNet được lựa chọn làm nền tảng cho mô hình phân loại.

ResNet~\cite{He2015DeepRL} là một trong những kiến trúc CNN hiệu quả nhất được đề xuất bởi He et al. (2015), nổi bật nhờ cơ chế \textcolor{red}{skip connection}, cơ chế này cho phép huấn luyện các mô hình có chiều sâu lớn hơn mà vẫn duy trì hiệu quả hội tụ, đồng thời hạn chế hiện tượng mất mát thông tin trong quá trình lan truyền ngược.

So với các kiến trúc phức tạp hơn như EfficientNet, Vision Transformer hay các mô hình kết hợp đa nhánh, ResNet có ưu điểm là dễ triển khai, ít tham số hơn, và có thể tùy chỉnh dễ dàng số lượng tầng để phù hợp với yêu cầu tính toán của hệ thống. Trong phạm vi nghiên cứu này, một phiên bản rút gọn từ ResNet-50~\cite{He2015DeepRL} được sử dụng nhằm đảm bảo cân bằng giữa độ chính xác và chi phí tính toán.
...
\begin{itemize}
	...
	\item \textbf{Khối Layer1 (Residual Block 1)}:
	\begin{itemize}
	...
	\item \textbf{Khối Layer2 (Residual Block 2)}:
	\begin{itemize}
...
Nhằm tối ưu khả năng triển khai thực tế trên các thiết bị có tài nguyên hạn chế, sau khi hoàn tất huấn luyện mô hình ở phần~\ref{sec:kien_truc_mo_hinh_rut_gon_tu_resnet_50}, ta tiếp tục áp dụng kỹ thuật Feature-Based Knowledge Distillation để rút gọn mô hình.
