...
%
%Trong phần này, tôi sẽ trình bày cơ sở lý thuyết của phương pháp xây dựng bộ lọc thông cao dựa trên việc biến đổi từ một bộ lọc thông thấp đơn giản – cụ thể là sbộ lọc trung bình. 
%%
...
\section{Bộ lọc thông cao trên không gian ảnh}
...
\label{sec:adof}
%Từ phương trình~\eqref{eq:y_n} mô tả bộ lọc thông cao dưới dạng tổng quát ta xây dựng được ma trận $H_x, H_y$ như bên dưới.
%\[
%H_x = \left[
%\begin{array}{rrr}
%	0 & 0 & 0 \\
...
Trong cài đặt, hệ số $\pm \tfrac{1}{2}$ được thay bằng $\pm 1$ để đơn giản hóa tính toán. Thực chất đây chỉ là nhân toàn bộ bộ lọc với một hằng số, nên kết quả \gls{convolution} chỉ thay đổi về thang đo mà không ảnh hưởng đến đặc trưng. Đây là cách chuẩn hoá hệ số thường dùng trong mạng nơ-ron tích chập.
...
Ngoài ra, phép nhân chập ảnh $I$ với bộ lọc $H_x$ là tương đương với việc 
...
Lựa chọn này có cơ sở ở chỗ các dấu hiệu phân biệt giữa ảnh thật và ảnh tạo sinh 
...
%	Phương trình~\eqref{eq:h_HP} mô tả bộ lọc thông cao dưới dạng tổng quát. Ta mở rộng thành ma trận $H_x, H_y$ như bên dưới để thuận tiện cho việc sử dụng thư viện như \texttt{Pytorch} khi lập trình (trong lý thuyết, hệ số chính xác phải là $\pm \tfrac{1}{2}$. 
%	Tuy nhiên, do trong quá trình xử lý ta chỉ quan tâm đến hướng và quan hệ tương đối của \gls{gradient}, 
%	nên có thể nhân với một hằng số tỉ lệ bất kỳ. 
%	Vì vậy, trong cài đặt ta thay hệ số $\pm \tfrac{1}{2}$ bằng $\pm 1$ để đơn giản hóa).
%	
%	\[
%		H_x = \begin{bmatrix}
%			0 & 0 & 0 \\
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=1.0\linewidth]{Images/adof_output_sample.png}
	\begin{minipage}{1.0\linewidth}
		\vspace{5mm}
		\caption{Hình ảnh trước và sau khi được xử lý bằng khối ADOF}
		\label{fig:adof_output_sample}
	\end{minipage}
\end{figure}
%
...
\section{Tối giản mô hình với kỹ thuật \glsentrytext{fbkd}}
%
