...
%%
%Bài toán phát hiện hình ảnh tạo sinh được tiếp cận dưới góc nhìn phân loại ảnh, trong đó mô hình cần học cách phân biệt giữa hai loại ảnh: ảnh thật (do máy ảnh chụp) và ảnh giả (được sinh ra từ các mô hình tạo sinh như GANs~\cite{Goodfellow2014GenerativeAN} hoặc Diffusion~\cite{Ho2020DenoisingDP}). Phương pháp đề xuất trong luận văn kết hợp giữa học sâu và kỹ thuật tiền xử lý ảnh nhằm tăng cường khả năng nhận diện các đặc điểm đặc trưng giúp phân biệt hai loại ảnh này.
%
%Cụ thể, trước khi đưa ảnh vào mạng học sâu, ảnh đầu vào sẽ được xử lý thông qua một bộ lọc tần số cao được thiết kế đặc biệt. Bộ lọc này hoạt động tương tự như một bộ lọc thông cao trong miền tần số, với mục tiêu làm nổi bật các chi tiết vi mô, nhiễu không gian, hoặc các dấu hiệu không tự nhiên thường xuất hiện trong ảnh tạo sinh. Thay vì sử dụng trực tiếp phép biến đổi Fourier~\cite{Arunachalam2013TheFF}, luận văn đề xuất một phương pháp tương đương trong miền không gian, giúp đơn giản hoá quá trình xử lý và tăng tốc độ tính toán, phù hợp với môi trường triển khai trên thiết bị tài nguyên thấp.
%
%Sau khi được tiền xử lý, ảnh sẽ được đưa vào một mạng học sâu với kiến trúc CNN~\cite{Krizhevsky2012ImageNetCW}, nhằm học các đặc trưng sâu phân biệt giữa ảnh thật và ảnh giả. Mô hình được huấn luyện trên tập dữ liệu bao gồm cả ảnh thật và ảnh tạo sinh từ nhiều mô hình khác nhau để đảm bảo tính tổng quát và khả năng phát hiện trên ảnh chưa từng gặp.
%
%Việc kết hợp giữa tiền xử lý đặc thù và mô hình học sâu cho phép hệ thống khai thác cả đặc trưng miền tần số lẫn đặc trưng không gian, từ đó nâng cao hiệu quả phát hiện ảnh giả trong nhiều điều kiện và nguồn ảnh khác nhau.
%%
%
